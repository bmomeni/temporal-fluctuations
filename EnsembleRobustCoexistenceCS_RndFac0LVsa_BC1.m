...
fpi = 0.1;
NOL = 0.7; % niche overlap factor
Fac = 0.1; % interaction coefficient in case of facilitation
...
rp = randperm(Ns0*Ns0-Ns0);
SwitchElements = rp(1:round(fpi*Ns0*(Ns0-1)));
cntw = 0;
for ii = 1:Ns0
    for jj = 1:Ns0
        if (ii~=jj)
            cntw = cntw+1;
            if min(abs(SwitchElements-cntw))==0
                ci(ii,jj) = Fac;
            end
        end
    end
...
save(strcat('EnsembleRobustCoexistenceCS_fpi',num2str(fpi),'_NOL',num2str(NOL),'_Fac',num2str(Fac),'_Rnd0LVsa_BC_Ngen',num2str(Ngen),'_fp',num2str(fp*100),'_Ne',num2str(Ne),'_rndseed',num2str(rndseed0),'.mat'))
