...
ciSS = zeros(Nsp,Nsp,Ne); % interaction coefficient matrices
NF = 11;
fluctuation_fraction = logspace(-3,-0.5229,11); % added by javad
Fluctuation_distance = zeros (Ne, NF); % added by javad

SS = zeros(Nsp,Nsp,Ne); % record of instances of interaction coefficients
...
for ne = 1:Ne
    ...
    % Individualizing the interaction coefficients for each strain
    ...
    td = 0;
    Gen = 0;
    while (Gen < Ngen)&&(Gen>=0) % between dilution steps
        ct = ct+1;
    ...
    %% sensitivity analysis of the measurements
    if Ncxst(ne)>1
        cF = 0;
        for F = fluctuation_fraction
            cF = cF + 1;
            S = S0i*Comp0;
            td = 0;
            count = 0;
            Gen = 0;
            cip_purturbed = cip .* ((1-F/2)+ F*rand(size(cip)));
            while (Gen < Ngen)&&(Gen>=0) % between dilution steps
                count = count+1;
                
                re = (r.*(1+1./K.*(cip_purturbed*S)).*((1+1./K.*(cip_purturbed*S))>0)).*(S>0);
                Su = S + dt*(re-d).*S; % updated pop. sizes because of growth
                td = td+dt;
            ...
            Fluctuation_distance (ne, cF)= CompDist(1,2);
        end
    end
    ...
    %     disp([N Ncxst(ne) DCS(ne)])
    %     toc
end

%% PLOT
%   hist(Fluctuation_distance, 10);
legend('Gaussian Purturbation amplitude = ' + string(fluctuation_fraction))

Fluctuation_cumulative = zeros(11,3);
for i = 1:6
    Fluctuation_cumulative (2:11,i) = cumsum(histcounts(Fluctuation_distance(:,i),10));
end
Fluctuation_cumulative = 100 - Fluctuation_cumulative/100;
% figure(1), hold on;
% %    truesize(figure(1), [100,100]);
% %    pbaspect ([1 1]);
% axis square;
% title('Results are Robust against Measurement Error');
% ax = gca;
% ax.TitleFontSizeMultiplier = 1;
% x = 0:.1:1; % assigning the x-axis as intervals of length 0.1 starting from 0.1 to 1;
% plot (x, Fluctuation_cumulative(:,1), '-', 'Color', 'r', 'LineWidth', 3);
% plot (x, Fluctuation_cumulative(:,2), '--', 'Color', 'k', 'LineWidth', 1.5);
% plot (x, Fluctuation_cumulative (:, 3:6), 'LineWidth', 2);
% set (gca, 'FontSize',15, 'FontName', 'Times');
% 
% lgd = legend('Purturbation Amplitude = '+string(fluctuation_fraction), 'FontSize', 10);
% %    lgd1 = text(.11, .05, 'Purturbation Amplitude = 0.003')
% %    set(lgd1, 'Rotation', -7, 'FontName', 'Times');
% %    lgd2 = text(.20, .06, 'Purturbation Amplitude = 0.03')
% %    set(lgd2, 'Rotation', -11, 'FontName', 'Times');
% xlabel('Deviation from the original composition', 'FontSize', 15);
% %    set(gca,'XTick',.1:.1:1 );
% ylabel('Percentage of cases with deviation', 'FontSize', 15);

cfd(1,:) = 1/sum(Ncxst>1)*sum((Fluctuation_distance<0.1).*(Ncxst'*ones(1,NF)>1));
c20 = 1/sum(Ncxst>1)*sum((Fluctuation_distance<0.2).*(Ncxst'*ones(1,NF)>1));
cfd(2,:) = c20 - cfd(1,:);
cfd(3,:) = 1 - c20;

for cf = 1:NF
    [phi,pci] = binofit(round(sum(Ncxst>1)*cumsum(cfd(:,cf))'),sum(Ncxst>1)*[1 1 1]);
    FfLCI(:,cf) = pci(:,1);
    FfHCI(:,cf) = pci(:,2); 
end
FfC = cumsum(cfd);
...
hold on
for cf = 1:NF
    bar([0 100*fluctuation_fraction(cf)],[zeros(3,1) cfd(:,cf)]',0.2*fluctuation_fraction(cf)^0.03,'stacked')
end
for ct = 1:3
    errorbar(100*fluctuation_fraction,FfC(ct,:),FfLCI(ct,:)-FfC(ct,:),FfHCI(ct,:)-FfC(ct,:),'k.')
end
% bar(100*fluctuation_fraction,cfd','stacked')
...
save(strcat('TestToleranceCoexistenceCS_RndExpSpt0LVsa_BC_Ngen',num2str(Ngen),'_Ne',num2str(Ne),'_fp',num2str(fp),'_rndseed',num2str(rndseed0),'.mat')) 
