...
%   and related variables. Proc. Natl. Acad. Sci. U.S.A. 103(51): 19430-19435.
%
% The indices for Morisita and Morisita-Horn were ported to Matlab
...
% Jul-2015: added 'eucn', replaced repmat with NaN
% Feb-2016: added 'cor3'
% Apr-2016: added support for 'vec'
...
switch method
   case {'euc','eucn','euc2','esq','eucs'} % skip for these methods
   otherwise
      idx = find(sum(X==0) == n);
      X(:,idx) = [];
end
...
switch method
   ...
   case {'cor2'}
      ...
      % Correlation Distance (Zapala & Schork, 2006 eq. 5):
   case {'cor3'}
      D = sqrt((1 - f_unwrap(corrcoef(X'),0))*2); % convert to distance
      
      % Euclidean Distance (eq. 7.34/7.35 in Legendre & Legendre, 1998):
   case {'euc','eucn','esq','eucs'}
      for i = 1:n-1 % repeat for each row except the last
         blk    = numel(i+1:n);              % get size of this block of distances
         r      = repmat(X(i,:),blk,1);      % extract this row, replicate
         R      = X(i+1:n,:);                % extract remaining rows
         ...
         if (isequal(method,'euc') || isequal(method,'eucn'))
            D(idx) = sqrt( sum( (r-R).^2 ,2) ); % sum by row produces a column
         else % 'esq'
            D(idx) = sum( (r-R).^2 ,2);         % sum by row produces a column
         end
      end
   ...
   case {'euc2'}
      for i = 1:n-1 % repeat for each row except the last
         ...
         % Exclude double-zeros (same effect as eq. 8 in Anderson et al., 2006):
         binZ    = ( (r>0)+(R>0) )==0; % binary matrix with 1 indicating double-zeros
         idxZ    = find(binZ==1);      % indices to elements with double-zeros
         r(idxZ) = NaN;                % replace double-zeros with NaN's
         R(idxZ) = NaN;
   ...
   case {'och'}
      ...
      % OCHIAI = W / sqrt(A*B)         -> denom is geometric mean
      for i = 1:n-1 % repeat for each row except the last
         blk    = numel(i+1:n);            % get size of this block of distances
         r      = repmat(X(i,:),blk,1);    % extract this row, replicate
         R      = X(i+1:n,:);              % extract remaining rows
...
switch method
   ...
   case {'eucn'}             % Euclidean distance normalized to 0-1
      dis = f_rewrap(D/max(D));
